...
std::string GrammarToDotStringVisitor::parentToChild(GExp* child) {
    ...
    std::string field_str = dot::fromFootList(nodeToDotID[parent_id_str],getChildCount());
    std::string child_id_str = std::to_string(getNodeID(child));
    incrChildCount();

    //std::string parent_node = getCurrParent();
    std::string parent_id_label = nodeToDotID[parent_id_str];
    std::string child_id_label  = nodeToDotID[child_id_str];

    //std::cout<<"parent: "<<parent_id_label<<", child:"<<child_id_label<<std::endl;

   // <<parent_id_str<<" "<<child_id_str<<std::endl;

    //std::cout<<"field string: "<<field_str<<std::endl;
 
    // Seq will have a list of children
    if (getCurrParent()!=NULL) {
        if (getCurrParent()->isaSeq()) {
            //std::cout<<field_str<<" "<<child_id_str<<std::endl;
            return dot::edgeSolid(dot::from(field_str), dot::to(child_id_label));

        // Otherwise, just assume parent is nodeWFoot
        } else {
            
            return dot::edgeSolid(dot::fromFoot(parent_id_label), 
                                  dot::to(child_id_label));
        }
    }
...
void GrammarToDotStringVisitor::outGrammar( Grammar* g,
   ...
   //nodeColor.push_back("#1CEBD5");
   //nodeColor.push_back("#B4E05C");
   //nodeColor[0] = "#1CEBD5";
   //nodeColor[1] = "#B4E05C";
...
void GrammarToDotStringVisitor::outEmpty( Empty* node ) {
    ...
    mStr += dot::nodeWHead(nodeToDotID[std::to_string(node_id)], "Empty", nodeColor[color]);
    //otherStr += label + ";"
...
// Epsilon node
void GrammarToDotStringVisitor::inEpsilon( Epsilon* node, std::list<std::shared_ptr<PTreeSym>> ptree_list) {
    int node_id = getNodeID(node);
...
void GrammarToDotStringVisitor::outEpsilon( Epsilon* node,
    ...
    mStr += dot::nodeWFoot(nodeToDotID[std::to_string(node_id)], 
                           debugDecorate("Epsilon",node), nodeColor[color] );
    //changing
...
void GrammarToDotStringVisitor::outSymLHS( SymLHS* node, NonTerm name,
    ...
    mStr += dot::nodeWFoot(nodeToDotID[std::to_string(node_id)],
                           debugDecorate(name.toString(),node), nodeColor[color]);
    //changing
...
void GrammarToDotStringVisitor::outTok(Tok* node, std::shared_ptr<Token> tok ) {
    ...
    mStr += dot::nodeDiamond(nodeToDotID[std::to_string(node_id)],
                             debugDecorate(tok->toString(),node), nodeColor[color]);
    //std::cout<<nodeColor[color];
...
void GrammarToDotStringVisitor::outAlt( Alt* node,
    ...
    if (getCurrParent()==node) {
        ...
        mStr += dot::nodeWFoot( nodeToDotID[std::to_string(node_id)],
                                debugDecorate("alt", node), nodeColor[color]);
        //std::cout<<nodeColor[color];
        //changing
...
void GrammarToDotStringVisitor::outDelta( Delta* node,
    ...
    mStr += dot::nodeWFoot(nodeToDotID[std::to_string(node_id)], 
                           debugDecorate("delta",node), nodeColor[color]);
